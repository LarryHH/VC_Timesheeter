...
class WindowForm():
    ...
    def fill_canvas_left(self):
        ...
        self.github_icon = PhotoImage(file=WindowForm.relative_to_assets(
    ...
    def fill_canvas_right(self, res, wb, date_sheet_name, date_cell):
        ...
        self.entry_monday = tkinter.OptionMenu(
            self.window, self.fm_options, *[first_monday, next_monday])
        ...
        sv_year = StringVar()
        sv_year.trace("w", lambda name, index, mode,
                      sv=sv_year: self.update_year_callbacks(sv))
        ...
        self.entry_template_label = self.create_entry_text_label(
            490, 94.8326416015625, "Template File"),
        self.entry_year_label = self.create_entry_text_label(
            490, 176.0, "Year"),
        self.entry_date_sheet_label = self.create_entry_text_label(
            490, 342.0, "Date Sheet"),
        self.entry_date_cell_label = self.create_entry_text_label(
            490, 425.0, "Date Cell"),
        self.entry_output_label = self.create_entry_text_label(
            490, 508.0, "Output Location"),
        self.entry_fm_label = self.create_entry_text_label(
            490, 259.0, "First Monday")
    ...
    def restore_labels(self):
        self.canvas.itemconfig(self.entry_template_label,
                               text='Template File', fill='black')
        self.canvas.itemconfig(self.entry_year_label,
                               text='Year', fill='black')
        self.canvas.itemconfig(self.entry_date_sheet_label,
                               text='Date Sheet', fill='black')
        self.canvas.itemconfig(self.entry_date_cell_label,
                               text='Date Cell', fill='black')
        self.canvas.itemconfig(self.entry_output_label,
                               text='Output Location', fill='black')
        self.canvas.itemconfig(self.entry_fm_label,
                               text='First Monday', fill='black')
    ...
    def update_year_callbacks(self, sv):
        ...
        for d in [first_monday, next_monday]:
            self.entry_monday['menu'].add_command(
                label=d, command=lambda d=d: self.fm_options.set(d))

    def button_processing(self):
        self.button_go['text'] = 'Processing...'
        self.button_go['font'] = tkinter.font.Font(
            family='VarelaRound Regular', size=24, weight='bold')
            ...
            for e in list(set(errs)):
                if e == 'FileExistsError' or e == 'OSError':
                    ...
                    self.canvas.itemconfig(
                        self.entry_output_label, text=msg, fill='red')
